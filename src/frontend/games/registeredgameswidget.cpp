...
RegisteredGamesWidget::RegisteredGamesWidget(QWidget* parent) : QWidget(parent) {
    createUI();
}
...
void RegisteredGamesWidget::addGame(const GameInfo& gameInfo) {
    ...
    connect(downloadButton, &QPushButton::clicked, this,
            [this, id = gameInfo.id]() { emit downloadModRequested(id); });
    ...
    connect(reloadButton, &QPushButton::clicked, this,
            [this, id = gameInfo.id]() { emit reloadGameRequested(id); });
    ...
    connect(deleteButton, &QPushButton::clicked, this,
            [this, id = gameInfo.id]() { emit deleteGameRequested(id); });
...
void RegisteredGamesWidget::updateGameStatus(int gameId, GameStatus status) {
    for (int i = 0; i < gamesList->count(); ++i) {
        ...
        if (item && item->data(GameIdRole).toInt() == gameId) {
            ...
            auto statusLabel =
                widget->findChild<QLabel*>("gameStatusLabel_" + QString::number(gameId));
            if (statusLabel) {
                QString statusText;
                QString statusColor;
