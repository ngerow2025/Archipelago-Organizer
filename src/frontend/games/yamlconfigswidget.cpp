...
YAMLConfigsWidget::YAMLConfigsWidget(QWidget *parent)
    : QWidget(parent)
{
    createUI();
}
...
void YAMLConfigsWidget::createUI()
{
    ...
    connect(createNewConfigButton, &QPushButton::clicked, this, &YAMLConfigsWidget::createNewConfigClicked);
...
void YAMLConfigsWidget::addConfig(const YAMLConfigInfo &configInfo)
{
    ...
    connect(editButton, &QPushButton::clicked, this, [this, id = configInfo.id]() {
        emit editConfigRequested(id);
    });
    ...
    connect(verifyButton, &QPushButton::clicked, this, [this, id = configInfo.id]() {
        emit verifyConfigRequested(id);
    });
    ...
    connect(quickLaunchButton, &QPushButton::clicked, this, [this, id = configInfo.id]() {
        emit quickLaunchRequested(id);
    });
    ...
    connect(deleteButton, &QPushButton::clicked, this, [this, id = configInfo.id]() {
        emit deleteConfigRequested(id);
    });
...
void YAMLConfigsWidget::updateConfigStatus(int configId, ConfigStatus status)
{
    for (int i = 0; i < configsList->count(); ++i) {
        ...
        if (item && item->data(static_cast<int>(ConfigRole::ConfigIdRole)).toInt() == configId) {
            ...
            auto statusLabel = widget->findChild<QLabel *>("configStatusLabel_" + QString::number(configId));
            if (statusLabel) {
                QString statusText;
                QString statusColor;
